...
|-------|---------|---------|--------|---------|
| **Panel Detector** | Panel Detection (boxes) | 1 (panel) | ⏳ **PENDING** | rcp_panel_v2+3 |
...
toProduction page
└─ content-crop → tile
└─ Panel Detector (FAST, high recall)
└─ panel box → OCR "P-105" (extract tag text from panel)
│
└─ [optional refine] ROI polygon extraction (closed-contour or ROI-seg)
├─ refined panel polygon ─┐
└─ quality checks (area/solidity/IoU)
│
fromProduction page
...
└─ tag box → OCR "P-105" ─┘
│
(panels paired by identical tag from OCR)
│
▼
...
|------|------------|------------------|
| Convert seg → det (panel polygon → panel box) | auto | ✅ From v2+3 seg |
| Build detection dataset YAML (rcp_bbox_v2+3) | 1 | ❌ New |
...
1.  **Panel Detector (Detection):**
    -   **Action:** Convert seg labels → panel-only boxes; train YOLOv8-s/m detect on merged v2+3.
    -   **Config (start):** imgsz=1280, batch=1, epochs=150, cos_lr=True.
    -   **Augs:** mosaic=0.7, auto_augment=randaugment, erasing=0.2, fliplr=0.25.
    -   **Output:** High-recall panel boxes to drive OCR for tag extraction.
    -   **Status:** ⏳ **PENDING**
2.  **RCP-Dual-Seg (Model 1):**
    -   **Action:** Continue current seg training with refined data and ROI feedback.
    -   **Status:** ✅ **ACTIVE**
3.  **Viewport + Tag detector (Model 2):**
    -   **Action:** Label 50 pages and train.
    -   **Status:** ⏳ **PENDING**

### **Phase 2: Specialized Models**
4. **Generate panel crops** → label dim_text + extract arrows
5. **Train Models 3 & 4** (Dim-Text, Arrow-Dir)

### **Phase 3: Integration**
6. **Build 500 pair dataset** → train Siamese classifier (Model 5)
