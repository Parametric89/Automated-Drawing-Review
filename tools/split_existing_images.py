...
------------------------
Split existing images from <base>/images/train/ into train/val/test with 70/20/10 ratio.
Also splits corresponding label files.
Moves images to fullsize folders in respective splits.
Use --base-dir to select a dataset root like datasets/rcp_dual_seg_v3.
"""
...
def split_images_train_val_test(source_dir: str,
...
def move_images_and_labels_to_splits(image_splits, base_dir: str):
    """
...
def save_split_info(image_splits, moved_count, base_dir: str):
    """Save split information for reproducibility."""
    train_images, val_images, test_images = image_splits
    
    split_info = {
        "dataset": Path(base_dir).name,
        "split_date": datetime.now().isoformat(),
        "split_ratios": {"train": 0.7, "val": 0.2, "test": 0.1},
        "source_directories": {
            "images": str(Path(base_dir) / "images" / "train"),
            "labels": str(Path(base_dir) / "labels" / "train")
        },
        "target_directories": {
            "images": {
                "train": str(Path(base_dir) / "images" / "train" / "fullsize"),
                "val": str(Path(base_dir) / "images" / "val" / "fullsize"), 
                "test": str(Path(base_dir) / "images" / "test" / "fullsize")
            },
            "labels": {
                "train": str(Path(base_dir) / "labels" / "train" / "fullsize"),
                "val": str(Path(base_dir) / "labels" / "val" / "fullsize"), 
                "test": str(Path(base_dir) / "labels" / "test" / "fullsize")
            }
        },
        "images": {
            "train": train_images,
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Split existing images into train/val/test and move to fullsize.")
    parser.add_argument("--base-dir", type=str, default="datasets/rcp_dual_seg_v3",
                        help="Dataset base directory (e.g., datasets/rcp_dual_seg_vX)")
    args = parser.parse_args()

    base_dir = args.base_dir

    print("=== Split Existing Images and Labels (70/20/10) ===")
    print(f"Splitting images and labels from {Path(base_dir) / 'images' / 'train'} into train/val/test with fullsize folders")
    print()
    ...
    # Step 1: Split images
    source_dir = os.path.join(base_dir, "images", "train")
    image_splits = split_images_train_val_test(source_dir=source_dir)
    ...
    moved_count = move_images_and_labels_to_splits(image_splits, base_dir=base_dir)
    ...
    save_split_info(image_splits, moved_count, base_dir=base_dir)
